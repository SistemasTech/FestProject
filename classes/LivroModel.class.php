 ...
 * @package entity
 */
class LivroModel extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='LivroModel';
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of LivroModel instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param LivroModel $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return LivroModel[]
	 */
	public static function findByExample(PDO $db,LivroModel $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of LivroModel instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	/**
	 * Will execute the passed statement and return the result as an array of LivroModel instances
	 *
	...
	/**
	 * returns the result as an array of LivroModel instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return LivroModel[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of LivroModel instances.
	 *
	...
	/**
	 * Fetch AluguerModel's which this LivroModel references.
	...
	public function fetchAluguerModelCollection(PDO $db, $sort=null) {
		$filter=array(AluguerModel::FIELD_CODIGOLIVRO=>$this->getCodigoLivro());
		return AluguerModel::findByFilter($db, $filter, true, $sort);
	}

	/**
	 * Fetch AutorModel which references this LivroModel. Will return null in case reference is invalid.
	 ...
	 * @return AutorModel
	 */
	public function fetchAutorModel(PDO $db, $sort=null) {
		$filter=array(AutorModel::FIELD_CODIGOAUTOR=>$this->getCodigoAutor());
		$result=AutorModel::findByFilter($db, $filter, true, $sort);
		return empty($result) ? null : $result[0];
	}

	/**
	 * Fetch CategoriaModel which references this LivroModel. Will return null in case reference is invalid.
	 ...
	 */
	public function fetchCategoriaModel(PDO $db, $sort=null) {
		$filter=array(CategoriaModel::FIELD_CODIGOCATEGORIA=>$this->getCodigoCategoria());
		$result=CategoriaModel::findByFilter($db, $filter, true, $sort);
		return empty($result) ? null : $result[0];
	}
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'LivroModel');
	}

	/**
	 * get single LivroModel instance from a DOMElement
	 *
	...
	/**
	 * get all instances of LivroModel from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('LivroModel') as $node) {
